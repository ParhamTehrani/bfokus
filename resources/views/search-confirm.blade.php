...
@section('content')
    <div class="container-fluid d-grid justify-content-center align-self-center align-items-center "  tabindex="-1">
        ...
        <div id="result" tabindex="0" aria-label="Get results for “{{ $search }}” with w-tap" onclick="search('{{ $search }}')" style="cursor:pointer;">
            <span id="output" style="display:none;" tabindex="-1">{{ $search }}</span>
            <p style="color: white;text-align: center;font-size: 2rem" tabindex="-1" id="wait-result">Get results for “{{ $search }}” with w-tap</p>
        </div>

        <a id="wait-btn" href="/?search=1&lang={{ request()->get('lang') }}&provider={{ request()->get('provider') }}" tabindex="0" aria-label="Voice search again."  style="cursor:pointer;display: flex;text-decoration: none"  >
            ...
            <p style="color: white;text-align: center;font-size: 2rem" tabindex="-1">Voice search again.</p>
        </a>
    </div>
